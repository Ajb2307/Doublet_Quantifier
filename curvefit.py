...
def fit_single_curve(spec, cont, dip, function="gaussian", show=False, continuum="linear"):
    '''
    ...
    if continuum == "linear":
        ...
        norm_spec_section[1, :] = spec_section[1, :] - \
            (m*spec_section[0, :] + b)
    ...
    fit_mask = ma.masked_inside(
        norm_spec_section[0, :], fitting_region[0], fitting_region[1]).mask
    ...
    if function == 'gaussian':
        ...
        A_bounds = [np.min(norm_spec_section[1, :]) * 2.5,
                    np.min(norm_spec_section[1, :]) / 5]
        mu_bounds = [dip[0], dip[1]]

        # fitting using scipy
        params, covariance = curve_fit(
            gaussian, norm_spec_section[0,
    ...
    if show == True:
        plt.plot(norm_spec_section[0, :], norm_spec_section[1, :],
                 label='Normalized Data', color='k', alpha=0.8)
        ...
        if show and function == 'gaussian':
            ...
            plt.plot(spec_section[0],
                     ...
                     label='Fitted Gaussian Curve', color='red', alpha=0.9, linestyle='--')

        # plotting if show == True and voigt
        if function == 'voigt':
            print("curve fit: A, σ, γ, μ: ", A_fit,
                  sigma_fit, gamma_fit, mu_fit)
            ...
            plt.plot(spec_section[0],
                     voigt(spec_section[0], A_fit,
                           sigma_fit, gamma_fit, mu_fit),
                     label='Fitted Voigt Curve', color='blue', alpha=0.9, linestyle='--')
        plt.legend()
        plt.show()
...
def two_curve_bounds(spec, cont, dip, continuum='linear', mus=None):
    ...
    A_bounds_g = [np.min(norm_spec_section[1, :]) * 2.5,
                  np.min(norm_spec_section[1, :]) / 10]
    A_bounds_g = [np.min(norm_spec_section[1, :]) * 100,
                  np.min(norm_spec_section[1, :]) / 10]
    ...
    params_g, covariance_g = curve_fit(
        two_gaussians, norm_spec_section[0,
                                         :][fit_mask], norm_spec_section[1, :][fit_mask],
        p0=[A0_g, A0_g, sigma0_g, sigma0_g, mu10_g, mu20_g],
        sigma=norm_spec_section[2, :][fit_mask],
        bounds=((A_bounds_g[0], A_bounds_g[0], sigma_bounds_g[0], sigma_bounds_g[0], mu1_bounds_g[0], mu2_bounds_g[0]),
                (A_bounds_g[1], A_bounds_g[1], sigma_bounds_g[1], sigma_bounds_g[1], mu1_bounds_g[1], mu2_bounds_g[1])))
...
def fit_two_curves(spec, cont, dip, function="all", show=False, mus=None, bin_continuum=True, bin_size = 10, bin_take = 'max'):
    ...
    if mus == None:
        ...
        mu1_0, mu2_0 = mus
        if mu1_bounds[0] >= mu1_0 >= mu1_bounds[1] or mu2_bounds[0] >= mu1_0 >= mu2_bounds[1]:
            raise ValueError(f"The either: {mu1_0} is not within the detected range [{mu1_bounds[0]}, {mu1_bounds[1]}] \
                             \n OR \n \
    ...
    if function == 'voigt' or function == 'all':
        ...
        try:
            params_v, covariance_v = curve_fit(
                ...
                sigma=norm_spec_section[2, :][fit_mask],
                bounds=((A_bounds_v[0], A_bounds_v[0], sigma_bounds_v[0], sigma_bounds_v[0], gamma_bounds_v[0], gamma_bounds_v[0], mu1_bounds[0], mu2_bounds[0]),
    ...
    if show == True:
        ...
        if function == 'gaussian' or function == 'all':
            ...
            #  label='Gaussian start parameters', color='grey', alpha=0.5, linestyle='--')
            plt.plot(spec_section[0], two_gaussians(spec_section[0], A1_fit_g, A2_fit_g, sigma1_fit_g, sigma2_fit_g, mu1_fit_g, mu2_fit_g),
                     label='Gaussian Curve fit', color='red',  alpha=0.7, linestyle='--')
        ...
        if function == 'voigt' or function == "all":
            ...
            #          label='Voigt start parameters', color='grey', alpha=0.5, linestyle='--')
            plt.plot(spec_section[0], two_voigts(spec_section[0], A1_fit_v, A2_fit_v, sigma1_fit_v, sigma2_fit_v, gamma1_fit_v, gamma2_fit_v, mu1_fit_v, mu2_fit_v),
                     label='Fitted Voigt Curve', color='blue', alpha=0.7, linestyle='--')
        ...
        if function == 'pseudo-voigt' or function == 'all':
            ...
            plt.plot(spec_section[0], two_pseudo_voigts(spec_section[0], nu1_fit_p, nu2_fit_p, A1_fit_p, A2_fit_p, sigma1_fit_p, sigma2_fit_p, mu1_fit_p, mu2_fit_p),
