...
def pseudo_voigts(x, nu, A=1, FWHM=1, mu=0):
    ...
    return I * ((nu * gaussian(x, ag, sigma, mu)) + ((1-nu) * lorentzian(x, mu, gamma)))
...
def two_gaussians(x, A1, A2, sigma1, sigma2, mu1, mu2):
    ...
    Returns:
        array-like: Gaussian distribution values at the specified x values.
    """
    y = (A1 * np.exp(-.5 * ((x - mu1) / sigma1)**2)) + \
        (A2 * np.exp(-.5 * ((x - mu2) / sigma2)**2))
...
def two_pseudo_voigts(x, nu1, nu2, A1, A2, FWHM1, FWHM2, mu1, mu2):
    ...
    return I1 * ((nu1 * gaussian(x, ag1, sigma1, mu1)) + ((1-nu1) * lorentzian(x, mu1, gamma1))) + \
        I2 * ((nu2 * gaussian(x, ag2, sigma2, mu2)) +
              ((1-nu2) * lorentzian(x, mu2, gamma2)))
